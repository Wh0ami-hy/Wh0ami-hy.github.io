...
一般流程是：用户向服务器发送用户名和密码。服务器验证通过后， 在当前对话（session）里面保存相关数据，比如用户角色、登录时间等。服务器向用户返回一个session_id，写进用户的Cookie。**用户随后的每一次请求，都会通过Cookie，将session.id传回服务器**。服务器收到session_id，找到前期保存的数据，由此得知用户的身份。
