...
`@Transactional`注解可以应用在类级别或方法级别。如果应用在类级别，整个类中的所有方法都将被视为一个事务单元。

**注意：**@Transactional 只能被应用到 public方法上，对于其它非public的方法，如果标记了@Transactional 也不会报错，但方法没有事务功能。

使用注解方式，只需在配置文件中加入一个`tx`标签来指定事务管理器，以告诉Spring 使用注解来完成事务的织入。
...
public class MyService {
    ...
    public void performTransactionalOperation() {
        // 在这里执行数据库操作，如插入、更新、删除等
        // 可以调用myRepository中的方法进行数据库访问
        // 事务会在方法结束时自动提交或回滚
    }
}
```
