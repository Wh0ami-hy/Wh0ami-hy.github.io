...
@Component 通常是通过类路径扫描来自动侦测以及自动装配到 Spring 容器中（我们可以使用 @ComponentScan 注解定义要扫描的路径，从中找出被@Component注解的 bean，自动装配到 Spring 的 bean 容器中）
...
## 13.5.Spring 事务的几种传播行为



## 13.6.Spring 事务的几种隔离级别

## 13.7.@Transactional(rollbackFor = Exception.class)注解
