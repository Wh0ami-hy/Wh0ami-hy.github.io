...
a-->LinkedList-双向链表
a-->Vector-数组
c-->HashSet-无序&唯一-底层是HashMap
c-->LinkedHashSet-底层是LinkedHashMap
...
其最大的特点是**允许保存有重复元素数据**，保存的顺序就是其存储顺序
...
## 27.1.比较 HashSet、LinkedHashSet 和 TreeSet
...
**HashSet适用于存储无序的唯一元素，常用于去重和集合运算**
...
从JDK 1.8开始，HashMap的实现出现了改变，HashMap内部类提供有一个重要的常量`static final int TREEIFY_THRESHOLD = 8`，当链表长度小于阈值（默认为 8）时，则使用链表形式存储，若超过，则将链表转为红黑树以实现树的平衡，保证数据的查询性能

## 33.4.HashSet和HashMap区别
...
Map.Entry 的主要作用就是作为一个Key 和 Value 的包装类型使用
