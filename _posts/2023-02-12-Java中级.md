...
其最大的特点是**允许保存有重复元素数据**，插入的顺序就是其存储顺序
...
应用场景：当需要一个基于数组的动态列表，支持随机访问和快速的插入/删除操作时
...
应用场景：当需要一个基于链表的动态列表，支持高效的添加/删除操作和迭代操作时
...
## 29.2. Collection集合转 Map

在使用 `java.util.stream.Collectors` 类的 `toMap()` 方法转为 `Map` 集合时，一定要注意当 value 为 null 时会抛 NPE 异常

## 29.3. Collection集合转数组

使用集合的 `toArray(T[] array)`，传入的是类型完全一致、长度为 0 的空数组

## 29.4. 数组转Collection集合
