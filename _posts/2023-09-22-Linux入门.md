...
挂载点：将一个文件系统与特定的目录进行关联，使得该文件系统中的文件和目录可以在该目录下进行访问，如 Linux系统中，根文件系统通常挂载在"/"目录下，而在Windows系统中，根文件系统则是挂载在特定的驱动器上（如C:\）

- `/`
- `/boot` 

`/boot`是用来引导的，一般系统会自动将其置于第一个分区`sda1`
...
主分区才能存数据，扩展分区不能直接存数据，需要再细分为逻辑分区才可以
