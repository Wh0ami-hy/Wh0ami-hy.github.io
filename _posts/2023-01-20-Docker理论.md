...
`WORKDIR <工作目录路径>`，**工作目录就是容器运行时所用的根目录**，后续的命令如COPY 和 RUN 都会在这个目录中执行。使用docker exec 进入容器后也会默认进入该目录。

WORKDIR 指令中需要使用绝对路径，如果指定的路径在镜像中不存在，WORKDIR 会自动创建该目录

当我们在 Dockerfile 中使用 WORKDIR 指定一个目录后，所有在此之后的指令都将在这个目录下执行，除非另有指定。这使得我们可以更方便地组织文件和命令，确保它们在正确的目录结构中运行

## 8.3. 复制项目文件到容器（COPY 或 ADD）
...
RUN指令执行命令，通常用于安装软件包或者配置环境。为了优化镜像大小和构建速度，建议合并多个RUN指令，并清理不必要的文件。
...
`VOLUME <路径>` 可实现挂载功能，可以在容器中挂载主机的目录，实现在容器中访问某目录就相当于访问主机的目录
