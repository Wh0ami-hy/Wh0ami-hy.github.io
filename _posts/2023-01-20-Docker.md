...
ADD：与COPY类似，但还支持从URL中下载文件，并且可以自动解压压缩文件。

WORKDIR：设置工作目录。

EXPOSE：声明容器运行时需要开放的端口。

ENV：设置环境变量。

CMD：设置容器启动后默认执行的命令。

ENTRYPOINT：设置容器启动后默认执行的命令，与CMD不同的是，ENTRYPOINT指定的命令不会被覆盖。

USER：设置容器中执行命令的用户。

VOLUME：声明持久化数据的目录，可以在容器中挂载主机的目录。

ARG：定义构建时的参数，可以用于在构建时传递变量
...
导出容器快照到本地文件，可以使用 `docker export` 命令
...
要实现一个 Web 项目，除了 Web 服务容器本身，往往还需要再加上后端的数据库服务容器，甚至还包括负载均衡容器等。
