...
* resultType：返回值类型。自动映射，用于实体类的属性名和表中字段名一致的情况  （必须设置）
* resultMap：自定义映射，用于一对多、多对一、字段名和属性名不一致的情况。和 resultType二选一
* parameterType：SQL语句中需要传的参数的类型。可以是类型的完全限定名或别名，如`int 或 java.lang.Integer`、`hashmap 或 java.util.HashMap`
...
### 5.1.1. 别名映射（resultType映射）

当查询的SQL返回的字段和Java实体类中的属性一样时，可用（结果是集合时也可用）

```xml
<select id="selectByname" parameterType="java.lang.String" resultType="com.hy.entity.Student">  
    select s_name as sName from student;  
</select>
```

### 5.1.2. 驼峰映射

Mybatis提供的驼峰命名映射的方式，如数据库中的`s_name`字段，能够自动映射到Java实体类中的`sName`属性。那么此时的查询的SQL变成如下即可：
...
resultMap 常用于数据库字段与实体类属性不匹配，resultMap 将数据库中的字段和JavaBean中的属性字段对应起来，实现一对一的映射
