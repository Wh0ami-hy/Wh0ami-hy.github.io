...
Java 中基本类型是值传递，对象类型传递的是引用的副本（本质仍是值传递，但表现类似引用传递）。

**值传递（Pass by Value）**

- **原理**：函数调用时，将实参的**值复制一份**传递给形参。函数内部操作的是这份 “副本”，而非实参本身。
- **特点**：函数内部对形参的修改**不会影响外部实参**。

引用传递是指在函数或方法中，**将参数的引用（内存地址）传递给函数或方法**。在函数或方法中对参数的修改会影响到原始值。这种传递方式常见于可变对象（例如数组、集合等）。

**引用传递（Pass by Reference）**

- **原理**：函数调用时，不复制值，而是将实参的**内存地址（引用）** 传递给形参。函数内部操作的是实参本身（通过引用访问）。
- **特点**：函数内部对形参的修改**会直接影响外部实参**。
